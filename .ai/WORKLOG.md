...
- `Modal` — `onClose` 유무로 닫기 버튼 · 오버레이 클릭 · ESC 동작이 갈리는 공용 껍데기
...
- 오버레이 클릭은 `event.target === event.currentTarget` 일 때만 닫는다 — 내용 영역 클릭으로 닫히면 안 된다
...
- 약관 실제 문구 — 현재 자리표시(`약관 내용 추가 예정입니다.`)
