...
브랜치: `fix/settlement-amount-input-block-sync` · 상태: 확인 완료
...
브랜치: `feat/finance-single-fetch` · 상태: 확인 완료
...
이슈: #TBD · 브랜치: `feat/employee-api-options` · 상태: 확인 완료
...
브랜치: `fix/ui-usability` · 상태: 확인 완료

화면에서 걸리던 자잘한 사용성 문제를 한 브랜치로 묶었다.
**담당이 갈린 곳은 손대지 않았다** — `features/project` · `features/issue` · `features/block`(정산 · 결재 블록 제외). 같은 문제가 남아 있으면 그쪽에서 따로 잡는다.
...
- **로딩 표시 통일** — 글자로 `불러오는 중…` 이라 적던 자리를 스피너로 바꿨다. 칸 안에 들어가는 자리는 작은 인라인 스피너를 쓴다
- **달력** — `showPicker()` 를 공통 속성으로 묶어 칸 아무 데나 눌러도 열리고, 같은 속성의 `min`·`max` 가 6자리 연도를 막는다
- **알림** — 헤더 드롭다운에서 잘리던 제목 · 내용을 다 보이게 하고, 좁은 화면에서는 화면 가운데로 띄운다
- **폼 오류** — 미입력 칸에 옅은 붉은 링을 더하고, 첫 오류 칸을 화면 **가운데로 옮긴 뒤** 포커스한다
- **부서 선택** — 하위를 가진 상위 부서는 고를 수 없고, 라벨이 `개발팀 (기술본부)` 라 고른 뒤에도 어느 본부인지 보인다
- **확인 모달** — 사원 등록(계정 발급 + 초기 비밀번호 메일)에 확인을 넣었다
- **호버** — 셀렉트 · 날짜 칸 · 알림 줄 · 달력 날짜 · 텍스트 버튼에 마우스 반응을 넣었다

### 트러블슈팅

- **결재 탭을 바꾸면 `상세` 버튼이 깜빡였다** — `scope` 는 URL 이라 즉시 바뀌는데 목록은 이전 탭 것이 남아 옛 줄이 새 탭 기준으로 그려졌다. 목록 유지 조건에 **탭이 같을 것**을 더했다
- **빈 상태 아이콘이 왼쪽에 붙었다** — `text-center` 는 줄글만 옮기는데 preflight 가 `svg` 를 블록으로 둔다. `mx-auto` 를 6곳에 넣었다
- **부서 `사원 보기` 가 비어 보였다** — 인원은 하위 포함인데 `GET /employees?departmentId=` 는 직속만 준다. 툴팁으로 직속 인원을 적고 백엔드에 옵션을 요청했다

### 부수 결정

- **개수를 모르는 자리에는 뼈대를 두지 않는다** — 결재 목록은 몇 줄이 올지 몰라 뼈대가 실제와 어긋난다. 그런 자리는 스피너를 쓴다
- **확인 모달은 되돌릴 수 없는 동작에만** — 첨부 제거 · 권한 회수처럼 다시 하면 되는 것은 넣지 않았다. 흔해지면 안 읽고 누른다
- **날짜 · 셀렉트 호버는 요소 타입에 건다** — 화면마다 클래스가 제각각이라 `globals.css` 한 자리에서 준다
...
- **로그인이 필요해 브라우저 확인을 못 했다** — `/files` 는 인증 가드가 로그인 화면으로 돌린다. `tsc` · `eslint` 만 통과 확인
...
## [2026-08-13] 프로필 사진 · 연락처 자동 하이픈 — 확인 완료
...
## [2026-08-13] 재무 — 입출금 CSV 일괄 등록 — 확인 완료
...
- **지역 코드는 프론트 상수** (`regions.ts`) — 목록 API 가 없다. 강원 `51` · 전북 `52` 는 특별자치도 전환 코드라 확인 필요
...
- **토스트를 컨텍스트가 아니라 전역 이벤트로** — 화면마다 프로바이더를 끼우지 않는다 (`issue:changed` 와 같은 방식). 네이티브 `<dialog>` 가 최상위 레이어라 토스트를 가린다 — 모달을 닫은 뒤 띄운다 (낙관적 처리라 자연히 그렇게 된다)
...
`AbortSignal` 을 받지 않는다 — 요청을 여럿이 나눠 쓰므로 한 화면이 떠났다고 끊으면 기다리는 다른 화면까지 실패한다. 대신 부르는 쪽이 `isStale` 플래그로 **결과를 버린다.**
...
- 화면 동작 확인 필요 — 그룹 CRUD · 구성원 추가/제거 확인 흐름 · 직급 인원수 패널
...
- 실제 등록은 **초기 비밀번호 메일이 발송되므로** 테스트 계정으로만 확인 필요
...
- 화면 동작 확인 필요 — 체크박스 연타 후 최종 상태 · 배치 편집 진입/이탈 · 설정 3개 화면(부서 · 직급 · 카테고리) 모달 · 사원 상세 모달 4개 · 이미지/문서 블록 모달
...
- 사이드바 렌더 결과(순서 · 라벨)는 사용자 화면에서 확인 필요
...
- `tsc --noEmit` · `eslint` · `next build` 통과
- 브라우저 확인은 못 했다 — ADMIN 로그인이 필요해 화면 동작은 미검증이다
...
- **`defaultColSpan` 을 2 → 1 로 내렸다** — 라벨 · 값을 한 줄씩 쌓는 구조라 2칸이 필요 없다. 표를 늘어놓는 다른 정산 계열(`입금 확인` · `세금계산서 조회`)은 2칸 그대로다. 이미 만들어진 블록의 `colSpan` 은 서버 값이라 안 바뀐다 (폭 변경 UI 가 없다)
...
- ** 색 분기를 CSS 클래스가 아니라 `isDark` prop 으로 되돌렸다.** 처음엔 `globals.css` 에 `.app-header` · `.header-profile*` 등 컴포넌트 클래스 9개(171줄)를 만들었는데, **한 곳에서만 쓰는 스타일이 전역 파일만 불린다.** 기존 토큰 유틸리티로 되돌리니 globals.css 추가분은 `.btn-danger` 14줄뿐이다
...
- 이슈 카드 `⋯` 메뉴에서 `~(으)로 이동`(상태 변경) 항목 제거 — **상태 변경은 드래그&드롭 하나로만** 한다
  ...
  - 드래그를 쓸 수 없는 사용자는 상태를 바꿀 수단이 없어진다
...
- **셸 높이 고정 — 헤더 · 사이드바는 스크롤에서 제외** (`AppShell.tsx` · `Sidebar.tsx` · `projects/[id]/layout.tsx`)
  ...
  - 중간 flex 래퍼에 `min-h-0` 이 없으면 자식이 내용만큼 부풀어 `overflow-y-auto` 가 걸린 영역이 넘치지 않는다 — 스크롤바가 안 생기는 원인이 된다
...
> 실제 동작 확인은 **백엔드 `PR #190` 머지 후** 가능하다.
...
> 실제 백엔드 대상 동작 확인은 **확인 필요** (`detail.txtId` · `detail.content` 가 내려와야 편집 가능)
...
> 실제 백엔드 대상 동작 확인은 **확인 필요** (스텝·블록 시드 데이터 필요)
...
- 약관 실제 문구 교체 (배포 전 필수 — 현재 placeholder)
...
- **약관 실제 문구 — 배포 전 필수.** 현재 자리표시(`약관 내용 추가 예정입니다.`) 상태로는 유효한 동의 기록이 아니다
...
> 위 양식을 복사해서 작업마다 새 블록을 **맨 위에** 추가하세요.
