...
- [x] `src/middleware.ts` 인증 가드 — 세션 쿠키 없으면 `/login`, 있으면 `/login` 진입 차단
- [x] 헤더 로그아웃 버튼 연결
...
- [x] 인증 가드 — 비로그인 시 `/` 접근 차단 (2026-08-04 등록 / 2026-08-04 `src/middleware.ts` 로 처리)
- [ ] 403 응답 시 `/forbidden` 이동 처리 (2026-08-04 등록)
- [x] 백엔드 세션 쿠키 이름 확인 → `SESSION` (2026-08-04 등록 / 2026-08-04 확인 후 `src/middleware.ts` 반영)
