...
현재 모든 `page.tsx` 는 **경로 확인용 스텁**(`<div>login</div>` 수준)이다.

### 공통 · 인증

| 경로         | 파일                     | 설명                                |
| ------------ | ------------------------ | ----------------------------------- |
| `/`          | `app/page.tsx`           | `/login` 으로 리다이렉트            |
...
>
> `FilterBar` · `SummaryCard` · `Loading` · `Empty` · `ErrorBox` 등은 **화면 요구사항과 UI 라이브러리가 정해진 뒤** 추가한다. 미리 빈 파일을 만들지 않는다.
...
| `menu.ts`      | 사이드바 · 설정 메뉴 정의      | 메뉴를 컴포넌트에 하드코딩하지 않는다                                                          |
| `status.ts`    | 상태 → 라벨 · 배지 색 매핑     | 키는 **백엔드 enum 값과 동일**해야 한다                                                        |
...
| `format.ts` | 금액 · 날짜 표기 포맷터 (`formatCurrency` · `formatDate` · `formatRelative`)                             |

> 두 파일 모두 지금은 **주석 가이드 + `export {}`** 만 있다. API 명세 확정 후 채운다.
