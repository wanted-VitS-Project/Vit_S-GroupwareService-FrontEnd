...
| **이슈 목록 · 상세에 `version` 이 오는지**       | 이슈 수정 · 상태 드래그 (없으면 400)                     | 55·57 |
| 이슈 상태 변경 응답에 `version` 이 실리는지      | 실리지 않으면 상태 변경 뒤 첫 수정이 재조회를 한 번 탄다 | 59    |
...
> (`src/features/block/BlockOwnerResignedContext.tsx` · 2026-08-12).
...
>
> ❗ **`version` 이 조회 응답(55 · 57번) 표에 아직 명시돼 있지 않다.** 계약은 나왔지만 실서버 확인 전이라
> 프론트는 `IssueSummary.version?` **선택 필드**로 받고, 값이 없으면 **수정 · 상태 변경을 막고 재조회를 안내한다.** 백엔드 확인 후 `?` 를 뗀다.
...
| `version`       | number                       | ❗ 확인 필요      |

> ℹ️ 목록에는 **`content` 가 없다.** 설명은 [57. 상세 조회](#57-이슈-상세-조회)로 받는다.
> ❗ **`version` 확인 필요.** 보드 카드가 상태 변경(59)에 이 값을 실어야 한다 — 없으면 드래그가 막힌다. ([이슈 도메인 — 공통](#이슈-도메인--공통))
...
> ⚠️ **낙관적 락** — 상태 · 완료 시각만 바꾸지만 **같은 `version` 조건**을 탄다. 어긋나면 409 다.
> ℹ️ 같은 상태로 다시 보내면 아무것도 바꾸지 않고 현재 값을 돌려준다. 단 **버전이 어긋나 있으면 그것도 409** 다 — 화면은 최신값을 읽어 **남이 이미 같은 상태로 옮겼으면 버전만 맞추고**, 다른 상태면 카드를 그 상태로 되돌린다.
