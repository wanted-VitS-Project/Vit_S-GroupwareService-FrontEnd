...
| [84](#84-정산-항목-수정-시-조회)          | 정산 수정 조회   | `GET /blocks/settlements/{id}/items`           | ✅ `features/settlement/api.ts`       |
| [85](#85-정산-항목-작성--수정)            | 정산 작성·수정   | `PATCH /blocks/settlements/{id}/items`         | ✅ `features/settlement/api.ts`       |
...
> ⚠️ **계좌번호는 응답 모양이 두 가지다.** 작성 응답(85번)은 `100******444` 로 **마스킹**되고,
> 수정 화면용 조회(84번)만 `originalAccountNumber` 로 원본을 준다. 폼에 채울 값은 84번 것이다.
> ⚠️ **추천 값(회차 · 총액)은 84번에만 있다.** 같은 프로젝트의 다른 정산 블록을 기준으로 계산되며,
> 블록 생성 직후 `수정하기` 로 들어갈 때만 의미가 있다 — 입력값이 아니라 **안내 문구**로 쓴다.
> ❗ **이미 저장된 항목의 현재 값을 주는 API 가 없다.** 84번은 추천값 · 원본 계좌번호만 준다 —
> 저장된 회차 · 금액 · 일자는 블록 목록(10번)의 `detail` 에서 읽어야 한다. **`detail` 스키마 확인 필요.**
> ❗ **블록 `type` enum 값과 `detail` 키(`settleId`) 확인 필요.** 9번 표의 9값에 정산이 없다 —
> "ERD enum 10값" 중 남은 1값으로 보인다.
...
| `recommendRoundNo`      | `number`         | 추천 회차 — 프로젝트 내 정산 블록 **개수** 기준         |
| `recommendTotalAmount`  | `number`         | 추천 총액 — 프로젝트 내 정산 블록 **총 금액** 기준      |
| `originalAccountNumber` | `string \| null` | 마스킹 없는 계좌번호. `OUTCOME` 이 아니면 `null`        |

| status | code      | 화면 처리                        |
| ------ | --------- | -------------------------------- |
| 403    | `IMG-007` | 접근 권한 없음                   |
| 404    | `IMG-003` | 존재하지 않는 블록               |

> ⚠️ **추천값은 입력값이 아니다.** 컬럼 안에 `추천: 2` · `추천: 4,500,000` 처럼 **안내로** 보여준다.
> ℹ️ 에러 코드가 `IMG-*` 다 — 이미지 도메인 코드를 그대로 쓰고 있다. **정산 전용 코드로 바뀔 수 있다.**
...
| ----------------- | ---------------- | ----------------------------------------------------------------- |
| `settleId`        | `number`         | 정산 블록 ID                                                      |
| `accountNumber`   | `string`         | ⚠️ **마스킹**된다 (`100******444`) — 원본은 84번에서만            |
...
| `paidAmountRatio` | `number`         | 금액 기준 진행률. 작성 직후 `0.0`                                 |
...
| 403    | —    | 편집 권한 없음                                                         |
| 404    | —    | 존재하지 않는 블록                                                     |
| 400    | —    | 빈 내용 · **출금인데 계좌 정보 누락** · `type` 누락                    |

> ❗ **문서와 예시가 어긋난다.** 상태 코드표는 `201 Created` 인데 성공 예시는 `httpStatus: 200` 이다.
> 프론트는 `2xx` 를 성공으로 보는 래퍼를 쓰므로 둘 다 통과한다 — 확인 필요.
> ❗ **`status` 표기 오타** — 문서 본문은 `PENDGING`, 예시는 `PENDING`. **예시를 따랐다.**
> ❗ **에러 `code` 가 비어 있다.** 지금은 백엔드 `message` 를 그대로 노출한다.
