 ...
 * | 블록 담당자             | `owner.deleted`(D-6) **+** `member.resigned` |
 * | 참여자 목록 기반 후보    | `member.resigned`            |
 *
 * ⚠️ 블록 일괄 조회에는 `resignedAt` 이 없다. `deleted` 만 보면 **퇴사했지만 사원 데이터가
 *    남은 담당자를 놓치므로**, 보드가 참여자 목록(45번)으로 보충한 값을 함께 본다 —
 *    둘 중 하나만 참이어도 문구를 붙인다 (`features/block/BlockMembersContext.tsx`).
