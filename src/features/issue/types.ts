// 스텝 이슈 도메인 타입·라벨·배지 색.
// 응답에 없는 것은 화면에서도 만들지 않는다 — 이슈 키·시작일·이슈별 진척도·활동 이력은 명세에서 제외됐다.
...
// 사원은 삭제되지 않고 퇴사일만 기록되므로, 퇴사자여도 항목을 빼지 않고 이름 뒤에 (퇴사자) 만 붙인다.
// 필드명은 deletedAt 이 아니라 resignedAt 이고, 블록 담당자의 deleted 와도 다른 값이다.
...
export interface IssueSummary {
  ...
  /** 낙관적 락 버전. 목록·상세·프로젝트 이슈 응답 모두에 실려 온다 */
  version: number;
}

// 이슈 진척도 — 프로젝트 전체·스텝별이 같은 모양이다.
// progressRate 는 이슈가 0개일 때 null 이다 — 0% 와 구분해야 "이슈 없음" 과 "하나도 못 끝냄" 이 안 섞인다.
...
// 시작 전(TODO) 이슈 수 — 서버가 주지 않아 전체에서 진행 중·완료를 빼서 만든다.
...
export interface CreateIssueRequest {
  title: string;
  content?: string | null;
  /** 생성만 yyyy-MM-ddTHH:mm:ss 다 (수정·조회는 날짜) */
  dueDate?: string;
  status?: IssueStatus;
  priority: IssuePriority;
  /** 사번 목록 */
...
// content·dueDate 는 null 이 "해제" 다. assigneeIds·blockIds 는 추가분이 아니라 최종 전체 목록이고 null 은 400 이다.
export interface UpdateIssueRequest {
  ...
  assigneeIds?: string[];
  blockIds?: number[];
  /** 최초 조회값(base)의 버전. draft 의 버전이 아니라 비교 기준의 버전이어야 하고, 어긋나면 409 다 */
  version: number;
}
...
export interface IssueStatusChanged {
  ...
  completedAt: string | null;
  updatedAt: string;
  /** 저장 후의 새 값. 상태 변경도 버전을 올리므로 카드에 덮어쓰지 않으면 다음 수정이 409 다 */
  version: number;
}

// 담당 이슈 캘린더 한 줄 (GET /issues/calendar).
// IssueSummary 와 모양이 다르다 — 담당자·연결 블록·version 이 없고 대신 프로젝트·스텝이 실려 온다.
// DONE 이슈와 마감일 없는 이슈는 응답에서 빠지므로 화면이 다시 거르지 않고, dueDate 도 null 이 아니다.
...
// 생성·수정 모달이 다루는 입력값.
// status 는 없다 — 생성은 항상 TODO 이고 이후 변경은 보드 드래그로만 한다. completedAt 은 서버가 관리한다.
...
// 부분 수정이 다루는 필드 전체. status·completedAt 은 별도 API 소관이라 충돌 비교 대상도 아니다.
...
// 담당자·블록 id 집합 비교. 순서만 다른 같은 집합은 변경으로 보지 않아 불필요한 재동기화를 막는다.
...
// 비교 전 앞뒤 공백을 다듬는다 — 저장할 때 어차피 떨어지는데 그대로 비교하면
...
// PATCH 본문 — 지정한 필드만 담는다 (null 은 해제 신호).
// 상세 객체 전체를 다시 보내지 않는다. 안 보낸 필드는 서버가 건드리지 않아,
...
// 마감일 오름차순 정렬 비교자 (미지정은 마지막). 서버가 정렬해주지 않아 화면에서 한다.
// 'YYYY-MM-DD' 는 사전순 = 시간순이라 문자열 비교로 충분하다.
// 첫 조회에서 한 번만 쓴다 — 이후에는 생성·상태 변경한 이슈가 열 맨 위로 오도록 배열 순서를 그대로 둔다.
...
// 마감일이 며칠 지났는지. 아직 안 지났거나 마감일이 없으면 0 이고, 완료된 이슈는 지났다고 보지 않는다.
// 달력 날짜 기준으로 센다 — 로컬 자정끼리 밀리초를 빼면 서머타임 전환일(23·25시간)에 하루가 밀려,
// 전환이 없는 Date.UTC 에 연·월·일만 넣어 정확한 날수 차이를 얻는다.
