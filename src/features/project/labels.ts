 ...
 *
 * ⚠️ **컴포넌트 파일에 두지 않는다.** 라벨을 화면 컴포넌트에서 내보내면
...
/**
 * 프로젝트 참여자 권한.
 * ⛔ `NONE` 은 폐기됐다 (2026-08-06) — 차단은 **참여자 제거**로 표현한다.
 */
...
/**
 * 스텝 권한.
 * ⚠️ **여기만 `NONE` 이 살아 있다** — 스텝 단위로는 "이 스텝만 가린다" 가 성립한다 (STP-011).
 */
...
/**
 * 종결 사유 라벨 — **응답이 `string` 인 자리**에서 쓴다.
 *
 * ⚠️ 상세 조회의 `closeReasonCode` 는 타입이 `string` 이라 단언(`as`)으로 좁히면
