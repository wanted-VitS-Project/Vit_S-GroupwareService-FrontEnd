...
// 스테이지 색인과 판정이 끝났는지를 함께 돌려준다.
// index 만으로는 아직 오는 중과 실패해서 영영 안 옴을 구분할 수 없다.
...
// - index 가 아직 없거나 실패했으면 묶지 않은 한 덩어리로 돌려준다
// 매 렌더 새 배열을 만든다 — 부르는 쪽에서 useMemo 로 감싼다.
