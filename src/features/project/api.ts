...
// 권한이 없는 프로젝트는 403 이 아니라 목록에서 빠진다 —
...
// 하드 삭제다 — project_member 에 soft delete 컬럼이 없다.
...
// 응답에 version 이 없다 — 생성 직후 수정하려면 상세를 다시 읽는다.
...
// 전체 덮어쓰기다 — 폼 전체를 매번 보낸다. 생략한 필드는 유지가 아니라 해제된다.
// 낙관적 락 — 409 PROJECT_VERSION_CONFLICT 면 재조회 / 덮어쓰기를 사용자에게 묻는다.
// 응답 version 은 저장 후의 새 값이라 화면 상태를 갈아끼워야 다음 저장이 통과한다.
...
// 종결해도 목록·활동 기록에서 사라지지 않는다 (PRJ-004) — 삭제와 다른 동작이다.
...
// 409 를 실패로 끝내면 그 프로젝트는 영영 삭제할 수 없다.
// 하위 스테이지·스텝·블록·이슈가 함께 논리 삭제되고 복구는 없다.
// 연결된 공고(bid_notice_id)는 비워진다 — 그 공고로 다시 만들 수 있다.
...
// 이미 붙은 것이 하나라도 섞이면 요청 전체가 409 다 — 호출 측이 걸러서 보낸다.
// 응답이 연결 후 전체 목록이라 화면 상태를 통째로 교체한다.
...
// 마스터가 삭제된 카테고리도 해제할 수 있다 — 연결 행은 그대로 남아 있다 (D-3).
...
// 참여자 전원의 최종 판정이 온다 — overridden: false 는 차단이 아니라
// 프로젝트 권한 상속이다 (STP-011). 응답이 { permissions: [...] } 라 여기서 벗긴다.
...
// 특정 스텝만 가리려면 NONE 행을 명시적으로 넣어야 한다 (STP-011) —
...
// 차단이 아니다. 응답 permission 은 회수 후 상속된 등급이다.
// 행이 원래 없으면 404 STEP_PERMISSION_NOT_FOUND — 이미 상속 상태라는 뜻이다.
...
// 이 스테이지에 새로 생길 스텝의 권한 기본값 저장. (.ai/API.md 128)
// 기본값은 권한 판정에 쓰이지 않는다 — 스텝 생성 시 복사될 뿐이고
...
// 스테이지 이름 수정.
// 낙관적 락이다. 그 사이 남이 먼저 저장했으면 409 STAGE_VERSION_CONFLICT 가 온다 —
...
// 스테이지 순서 변경 (STG-002).
// 전체 최종 순서를 보낸다 — 일부만 보내면 나머지와 sort_order 가 겹친다.
// 낙관적 락은 항목마다 검사하고, 하나라도 어긋나면 요청 전체가 409 로 롤백된다.
...
// 전체 덮어쓰기다 — 폼 전체를 매번 보낸다. 생략한 필드는 해제된다.
// 낙관적 락 — 409 STEP_VERSION_CONFLICT 면 재조회 / 덮어쓰기를 묻는다.
...
// 위치를 바꾸는 유일한 경로다 — 스텝 수정(PATCH /steps/{stepId})은 stageId 를 받지 않는다.
// 보드 전체의 최종 배치를 보낸다. 낙관적 락은 항목별이고 실패하면 전체 롤백이다.
// 선행 스텝 완료를 검사하지 않는다 — 끌어 놓은 대로 저장된다.
// 2026-08-11 BE 확인 3건:
// - sortOrder 는 프로젝트 단위 통번호다 — 스테이지마다 1부터 다시 세지 않는다.
...
// 스텝 삭제.
// moveBlockIds 를 비우면 하위 블록을 전부 삭제한다. 살릴 블록만 골라 넘기면
...
// 낙관적 락 — 409 STEP_VERSION_CONFLICT 면 재조회 / 덮어쓰기를 묻는다.
// DONE 에서 되돌리면 완료 기록(completedAt·completedBy)도 함께 비워진다.
// 스텝 상태는 진척률과 별개 값이다 (STP-004).
...
// 멱등이다 — 이미 완료된 스텝을 다시 완료해도 완료자·완료시각을 덮어쓰지 않는다.
