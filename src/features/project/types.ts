...
export interface BusinessCategory {
  ...
  // 조회 응답(상세·목록)에만 있다 — 연결 API(132) 응답에는 없다.
  // 삭제돼도 이름은 그대로 오고 연결 행도 남으므로 배지로 표시하고 해제는 허용한다.
  deleted?: boolean;
}
...
// permission 은 VIEWER·EDITOR 2값이다 — NONE 은 폐기됐다 (2026-08-06).
...
export interface ProjectMember {
  ...
  // 퇴사(resigned)와 다른 상태다 — 둘 다 참일 수도, 하나만 참일 수도 있다.
  // true 여도 이름은 그대로 오고 행도 사라지지 않는다 (예전 INNER JOIN → 지금 LEFT JOIN).
  // 조회에는 보이지만 쓰기는 받지 않는다 — 권한 변경을 부르면 USER_NOT_FOUND 404 다.
  deleted?: boolean;
}

// GET /api/v1/projects/{projectId}
// status 는 백엔드 enum 이 확정되지 않아 string 으로 둔다. (예: 'IN_PROGRESS')
export interface ProjectDetail {
  ...
  /** YYYY-MM-DDTHH:mm:ss */
  createdAt: string;
  // 낙관적 락 버전 — 화면에 그리는 값이 아니라 다음 쓰기에 실어 보낼 값이다.
...
// 상세(ProjectDetail)와 달리 stepCount·doneStepCount·description 이 없다.
...
export interface ProjectListItem {
  ...
  // 이름이 InProgress 라고 그 상태만 세는 것이 아니다 — REJECTED 도 포함이다.
  // (2026-08-11 myApprovalOpenCount 에서 개명. 뜻은 그대로다)
  myApprovalInProgressCount: number;
}

// 목록 조회 파라미터.
// 정렬은 created_at DESC → project_id DESC 고정이라 정렬 파라미터가 없다.
...
// 낙관적 락에 쓰는 버전 값.
// 선택 필드로 둔다. 수정 API 2종은 version 이 필수(누락하면 400)인데
// 목록 조회 명세(API.md 7·8번)에는 아직 이 필드가 없다 — 낙관적 락이 2026-08-11 신설이라
// 목록 응답 반영 여부가 확인되지 않았다. 값이 없으면 화면이 수정 저장을 막고 재조회를 안내한다.
// 백엔드에서 확인되면 ? 만 떼면 된다.
...
// 순서 변경의 orders[].stageId 와 규약이 다르다 — 그쪽 미소속은 null 이다 (2026-08-11 BE 확인).
...
// 순서 변경 요청 한 줄.
// 낙관적 락을 항목마다 검사한다. 하나라도 어긋나면 요청 전체가 409 로 롤백된다 —
// 부분 적용은 없다. 이 API 에는 overwrite 가 없어 409 면 재조회 후 다시 끌어야 한다.
// 전체 최종 순서를 보내야 한다. 일부만 보내면 보내지 않은 항목과 sort_order 가 겹친다.
...
export interface StepOrderItem {
  ...
  // 항상 보낸다 — 보드 전체의 최종 배치를 싣는 API 라 생략할 자리가 없다.
  stageId: number | null;
  // 프로젝트 단위 통번호다 (2026-08-11 BE 확인) — 스테이지마다 1부터 다시 세지 않는다.
...
// 회사(company_id)는 로그인 사용자 것이 자동으로 박힌다 — 요청으로 지정할 수 없다.
// 상태는 시스템이 NOT_STARTED 로 정하고, 생성자는 자동으로 EDITOR 참여자가 된다.
...
// version 이 없다 (2026-08-11) — 새로 만든 프로젝트의 version 은 1 이다.
// 생성 직후 곧바로 수정하려면 상세(6)를 다시 조회한다.
// businessCategories[].deleted 는 쓰기 경로라 항상 false 다 —
...
// 전체 덮어쓰기다. 생략한 필드는 유지가 아니라 해제된다 — 선택 필드라는 것은
...
// 연결 후 전체 목록이 온다 — 방금 추가한 것만이 아니라서 화면 상태를 통째 교체한다.
// 이 응답에는 deleted 가 없다 (살아 있는 것만 고를 수 있어서다).
...
// 참여자 권한과 달리 NONE 이 살아 있다 — 스텝 단위로는
...
// 기본값은 권한 판정에 쓰이지 않는다 — 스텝 생성 시 step_permission 으로 복사될 뿐이고,
