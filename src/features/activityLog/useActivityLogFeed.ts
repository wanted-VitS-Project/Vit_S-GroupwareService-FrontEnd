...
// 활동 기록 목록 + 커서 이어 읽기 — 스텝 화면과 블록 팝업이 같은 규칙을 쓴다.
// blockId 를 주면 그 블록의 기록만 본다 (블록 전용 API 는 없다).
// 조건이 바뀌어도 목록을 지우지 않는다 — 직전 목록을 띄운 채 새 조건을 부르고 도착하면 갈아끼운다.
export function useActivityLogFeed(
  stepId: string,
  blockId?: number,
  options?: {
    /** 감시 지점을 재는 스크롤 영역. 팝업처럼 자체 스크롤을 가진 곳에서 넘긴다 (없으면 viewport 기준) */
    root?: Element | null;
  },
) {
  ...
  useEffect(() => {
    ...
    getStepActivityLogs(stepId, { blockId, signal: controller.signal })
      .then((page) => {
        // 조건이 바뀐 뒤 도착한 응답은 버린다 — 취소를 걸어 뒀어도 이미 도착했으면
        // 그대로 통과해 지난 조건의 첫 페이지가 새 목록을 덮는다.
  ...
  // 이어 읽기가 나가 있는지. 감시 지점은 스크롤 중 여러 번 걸려 state 로만 막으면 같은 커서를 두 번 부른다.
  const loadingMoreRef = useRef(false);
  // 나가 있는 이어 읽기 요청 — 조건이 바뀌면 끊는다.
  const loadMoreAbortRef = useRef<AbortController | null>(null);

  // 조건이 바뀌면 나가 있던 이어 읽기를 끊고 진행 상태를 되돌린다.
  // 없으면 A 조건 요청이 끝날 때까지 loadingMoreRef 가 잠겨 B 조건의 감시 지점 요청이 무시되고,
  // 감시 지점은 "보이기 시작할 때" 한 번만 알리므로 자동 이어 읽기가 그대로 멈춘다.
  useEffect(() => {
    return () => {
