// 스텝 활동 기록 도메인 타입·표시 사전.
// 서버는 완성된 문장을 주지 않는다 — 조립에 필요한 원자 데이터만 오고 문장·날짜 그룹·상대 시간은 화면에서 만든다.
...
// 퇴사자여도 기록을 지우지 않고 이름 뒤에 (퇴사자) 문구만 붙인다 (components/PersonNote.tsx).
...
export interface ActivityLog {
  ...
  actor: ActivityActor;
  block: ActivityBlock;
  /** 'YYYY-MM-DDTHH:mm:ss' — 타임존 표기가 없다 (time.ts 에서 로컬로 해석) */
  createdAt: string;
}
...
// 수정 필드 이름표. 사전에 없는 필드가 오면 원래 값을 그대로 보여준다 — 빈칸으로 두지 않는다.
...
// 값 사전. fieldName 별로 원자 값을 사람이 읽는 말로 바꾼다. 사전에 없으면 원래 값을 그대로 쓴다.
...
// 변경 값 표시 방식 — title·content·caption 만 펼쳐 보이고(expand) 나머지는 한 줄에 붙인다(inline).
...
// 원자 값을 화면 문구로 바꾼다. 값이 없으면 빈 문자열을 준다 (호출 측에서 '없음' 처리).
...
// 필터 옵션의 '전체' 값. blockId 를 빼고 조회한다는 뜻이라 숫자와 섞이지 않게 문자열로 둔다.
