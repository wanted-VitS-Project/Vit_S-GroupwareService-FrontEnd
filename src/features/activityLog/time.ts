...
// 서버 값에 타임존 표기가 없어(2026-08-02T14:32:00) 그대로 new Date() 에 넣으면
// 브라우저마다 UTC 로 읽을 수 있다. 그래서 직접 쪼개서 로컬 시각으로 만든다.

// 'YYYY-MM-DDTHH:mm:ss' — 초·소수점 이하는 없어도 받는다.
// 끝까지 검사한다 — 뒤가 열려 있으면 타임존이 붙은 값(...Z·+09:00)까지 받아 시각이 어긋난다.
...
// createdAt 문자열을 날짜 그룹 키·라벨·시각·상대 시간으로 풀어낸다.
...
export function parseActivityTime(
  ...
  // Date 는 범위를 벗어난 값을 조용히 넘긴다 (2월 30일 → 3월 2일).
  // 그러면 날짜 머리와 상대 시간이 서로 다른 날을 가리켜, 넣은 값과 나온 값이 모두 같을 때만 인정한다.
  const isSameMoment =
...
// '방금'·'N분 전' 같은 상대 표기. 하루를 넘겼거나 미래 시각이면 시각을 그대로 보여준다.
...
// 기록을 날짜별로 묶는다. 응답이 이미 최신순이라 순서를 다시 세우지 않고 훑으며 자른다.
