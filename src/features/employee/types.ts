...
/** 목록 필터의 상태 값. 계정 상태와 비밀번호 상태를 서버가 하나로 받는다 */
...
export interface EmployeeSummary {
  ...
  /**
   * 사진 서빙 경로. 사진이 없으면 null 이다.
   * presigned 가 아니라 만료되지 않는 우리 경로라 그대로 img src 에 넣는다.
   */
  profileImageUrl: string | null;
}

/** 사원 상세. 목록 필드에 수정 폼 초기값이 더 붙는다 */
...
/**
 * 이름 검색 결과 한 명. 결재선 지정용이다.
 * 인사관리 목록과 필드 이름이 다르고 민감 정보 없이 네 개뿐이다.
 */
...
export interface CreateEmployeeResult {
  ...
  emailRegistered: boolean;
  /** 메일 발송이 실패해도 201 이다. false 면 재발송해야 로그인할 수 있다 */
  emailSent: boolean;
}

/**
 * 사원 정보 수정. 전달한 필드만 바뀐다.
 * 키를 빼면 변경 안 함, null 을 보내면 미지정으로 지움이다.
 */
export interface UpdateEmployeeRequest {
  ...
  hiredAt?: string;
  /**
   * 학력 · 자격증은 부분 수정이 아니라 전체 교체다.
   * 생략하면 기존 유지, 빈 배열이면 전부 삭제라 폼이 편집 여부를 구분해 보낸다.
   */
  educations?: EducationValue[];
  certificates?: CertificateValue[];
}
...
/** 퇴사 처리 응답. 퇴사일 기록과 계정 비활성이 한 번에 일어난다 */
 ...
 * 등록 직후 응답(메일 실패 재발송)도 그대로 넘길 수 있어야 한다.
 */
...
export interface PasswordResetFailure {
  ...
  /** true 면 비밀번호는 바뀌었고 메일만 실패했다. 재발송해야 로그인할 수 있다 */
...
/**
 * 엑셀 일괄 등록의 행 오류.
 * 검증과 등록이 같은 구조라 등록 후에도 같은 표로 보여준다.
 */
...
/**
 * 일괄 등록 검증 결과. 오류 행이 있어도 200 이라 errorCount 로 분기한다.
 * 400 은 파일 자체 문제 3종뿐이다.
 */
