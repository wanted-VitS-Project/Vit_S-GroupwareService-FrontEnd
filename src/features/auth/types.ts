...
export interface CurrentUser extends LoginResponse {
  ...
  /**
   * 아바타 서빙 경로. 사진이 없으면 null 이다.
   * 만료되지 않는 경로라 프론트에 캐싱 · 재발급 로직을 두지 않는다.
   */
...
/**
 * 업로드 제약. 서버가 다시 검증하지만 헛된 왕복을 막으려 화면에서 먼저 거른다.
 * webp 는 서버가 검사할 수 없어 제외한다.
 */
