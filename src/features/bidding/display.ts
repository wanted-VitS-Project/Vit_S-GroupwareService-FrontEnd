...
/** 공고 검토 상태 — 전환 여부(`projectId`)와 **다른 축**이다 */
...
/**
 * 마감 배지. `dDay` 는 **서버가 계산한 값**이라 그대로 읽는다.
 *
 * ```
 * dDay > 0  → D-3
 * dDay = 0  → D-Day
 ...
 * ```
 *
 * 마감일이 없는 공고(수동 등록 등)는 배지를 만들지 않는다 — `none` 이면 화면이 비워 둔다.
 */
 ...
 *
 * 값이 없으면 `-` 다 — `0원` 으로 그리면 **금액이 0인 공고**와 구분되지 않는다.
 */
 ...
 * 목록 표처럼 폭이 좁은 곳에서만 쓰고, 상세에서는 `formatAmount` 를 쓴다.
 */
...
/** 값이 없는 칸은 빈칸이 아니라 `-` 로 채운다 — 빈칸은 로딩 중으로 읽힌다 */
