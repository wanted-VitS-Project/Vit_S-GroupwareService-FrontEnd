 ...
 * 401 · 403 은 lib/api.ts 가 전역으로 받아 여기서 다루지 않는다.
 */
 ...
 * 직접 등록한 공고는 사람이 읽을 번호가 없어 그리지 않는다.
 */
 ...
 * 수집처가 안 준 값을 불가 로 그리면 없는 사실을 지어내는 셈이다.
 */
...
/**
 * 입찰 공고 상세 화면.
 * 목록과 달리 금액을 줄이지 않는다. 3.4억 이 아니라 340,000,000원 이다.
 */
export default function NoticeDetail({ noticeId }: { noticeId: number }) {
  ...
  /**
   * 제외 · 복구 결과를 화면의 공고에 곧바로 반영한다.
   * 응답이 바뀐 값을 그대로 줘, 다시 부르면 스켈레톤으로 한 번 내려갔다 올라온다.
   */
  ...
  return (
    <>
      ...
      <div className="mt-2">
        {/**
         * 배지를 제목 옆에 둔다. 위에 한 줄을 따로 쓰면 공고명이 아래로 밀린다.
        ...
        <p className="mt-1.5 text-caption text-text-secondary">
          ...
          {/* 번호 없는 공고에 공고번호 - 를 붙이면 빠진 값처럼 읽혀 통째로 뺀다 */}
      ...
      {/* 제외 사유는 있을 때만. 빈 줄을 두면 제외가 아닌 것처럼 읽힌다 */}
      ...
      <div className="mt-6 flex flex-col gap-4 lg:flex-row">
        <div className="flex min-w-0 flex-1 flex-col gap-4">
          ...
          <Card title="금액">
            <Fields>
              ...
              />
              {/* 변동 폭 · 하한율은 수집처 원문이라 파싱하지 않고 그대로 보여준다 */}
              <Field
                label="예정가격 변동폭"
        ...
        <div className="flex w-full shrink-0 flex-col gap-4 lg:w-80">
          {/**
           * 직접 등록한 공고에는 이 카드를 두지 않는다.
           * 원문 링크도 첨부도 없어 없다는 안내만 두 줄 남는다.
           */}
          ...
          {/* 직접 등록한 공고만 고칠 수 있다. 수집 공고는 서버가 409 로 막는다 */}
          {notice.sourceCode === 'MANUAL' && (
            <Card title="공고 관리">
              <Link
                href={BIDDING_ROUTES.edit(noticeId)}
          ...
          <Card title="분석 · 전환">
            ...
            {/**
             * AI 검토는 요약과 다른 기능이다.
             * 공고 첨부와 사내 문서를 비교하고 결과에 근거 인용이 붙는다.
             */}
            <button
              type="button"
            ...
            {/**
             * 여기에는 전환 버튼을 두지 않는다. 전환은 완료된 AI 검토가 근거로 필요하다.
             * 진입점을 검토 결과 화면 한 곳으로 두어 순서가 뒤집히지 않게 한다.
             */}
      ...
      {showsSummary && (
        <Modal
          ...
          /* 검토 모달과 같은 크기로 맞춘다. 두 창을 오가며 쓰는데 폭이 다르면 흔들린다 */
          className="flex max-h-[85vh] w-full max-w-160 flex-col overflow-hidden rounded-base border border-border-default shadow-2xl"
          /**
           * 제목 줄을 직접 그린다.
           * 기본 헤더는 패널의 여백을 빌려 쓰는데 여기서는 패딩을 없애 글자가 모서리에 붙는다.
           */
          header={
        ...
        >
          {/* 요약이 길어 모달 안에서만 스크롤한다. 뒤 화면까지 굴러가면 어지럽다 */}
          <div className="min-h-0 flex-1 overflow-y-auto px-6 pt-5 pb-8">
            {/**
             * 모달이 이미 카드라 껍데기를 한 겹 더 씌우지 않는다.
             * key 로 공고가 바뀌면 새로 만든다. 아니면 앞 공고의 요약과 폴링이 남는다.
             */}
...
/**
 * 첨부 목록. 우리 저장소 파일이 아니라 원문 사이트 링크다.
 * hasAttachment 가 true 여도 목록이 빌 수 있어 배열 길이로 판단한다.
 */
function AttachmentList({ attachments }: { attachments: NoticeAttachment[] }) {
  ...
  return (
    /**
     * 파일명이 길어 전부 잘리므로 파일마다 테두리를 둘러 경계를 만든다.
     * 아이콘은 링크 여부를 함께 알린다.
     */
        ...
        <li key={attachment.attachmentOrder} className="min-w-0">
          {attachment.sourceUrl ? (
            <a
              ...
              // title 은 접근 이름에 들어가지 않아 새 탭 안내를 라벨로 준다
            ...
            /* 링크가 없는 첨부는 눌러도 갈 곳이 없어 테두리를 흐리게 둔다 */
            <span
              title={attachment.fileName}
...
/**
 * 제외 사유 입력 창.
 * 사유는 서버가 요구하는 필수 값이라 보내기 전에 화면에서 막는다.
 */
