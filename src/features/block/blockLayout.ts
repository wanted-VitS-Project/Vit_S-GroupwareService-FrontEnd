...
/**
 * 서버 응답을 `rowIndex` · `sortOrder` 순의 **평면 배열**로 편다.
 *
 ...
 *
 * 커서가 대상의 왼쪽인지 오른쪽인지를 보지 않고 **끌어온 방향**으로 정한다 —
 * 뒤에서 왔으면 대상 앞, 앞에서 왔으면 대상 뒤. 결국 둘이 자리를 맞바꾼다.
 *
 * ⚠️ 좌우 절반으로 앞/뒤를 정하면 **아무 일도 안 일어나는 사각지대**가 생긴다.
...
/**
 * 화면에 그려진 행을 그대로 서버 좌표로 옮긴다 — **보이는 배치가 저장되는 배치**다.
 *
