...
// CSR - 이미지 블록: 첫 장은 블록 목록 조회의 detail 로 함께 와 카드가 뜨자마자 그린다.
// 두 번째 장부터는 목록 조회 API 가 없어 서버가 현재 정렬 번호 + 방향으로 한 장씩만 준다 —
// 좌우 이동마다 한 번씩 부르고 받아 온 장은 정렬 번호로 캐싱해 되돌아올 때는 요청하지 않는다.
// 순서 편집 모달만 전체 목록이 필요해 그때 따로 모은다.
...
// 어느 이미지를 받아 올지 — 서버가 준 정렬 번호와 방향으로 지정한다.
// from 에 지어낸 값을 넣지 않는다. 정렬 번호는 1부터라 0 은 서버에 없는 자리이고,
// 한 번 어긋나면 다시 시도해도 같은 값을 보내 복구되지 않는다.
...
export default function ImageBlock({
  ...
  // 목록을 통째로 다시 받아야 하는 횟수. 정렬 번호를 모르는 상황이 셋 있다 —
  // 진입 때 대표 이미지가 안 실려 온 경우·다시 시도·삭제 직후(응답이 남은 이미지를 안 준다).
  // 셋 다 번호를 지어내지 않고 목록을 받아 서버가 준 번호로 맞춘다.
  // 이 조회는 편집 권한이 필요해 열람 전용 사용자는 여기까지 오지 않는다.
  ...
  // 지금 유효한 재동기화가 몇 번째인지. 목록 요청이 날아가 있는 동안 업로드가 끝날 수 있어,
  // 늦게 도착한 업로드 전 목록이 캐시를 갈아엎으면 방금 올린 이미지가 사라진다 —
  // 세대가 어긋난 응답은 버린다.
  ...
  // 생성 응답에는 새로 올린 것만 담기므로 기존 캐시를 비우지 않고 합치고,
  // 장수도 알던 값에 더한다 (몰랐으면 계속 모르는 채로 둔다).
  function applyCreated(created: BlockImage[]) {
    ...
    if (wasResyncing) {
      // 버린 목록이 들고 있던 장수를 모른다 (삭제 직후였을 수도 있다).
  ...
  // 좌우 이동. wrap 이면 마지막 → 첫 장, 첫 장 → 마지막으로 넘어간다.
  ...
  function go(step: 1 | -1, wrap = false) {
    ...
    // 이웃 장만 서버에 부른다 — from 이 지금 보고 있는 장의 진짜 정렬 번호라 안전하다.
    // 양 끝을 건너뛰는 순환 이동은 방향으로 표현할 수 없어 캐시에 있을 때만 순환한다.
    // 첫 장은 블록 응답으로 이미 받아 두었고 목록을 다시 받은 뒤에는 전부 캐시에 있다.
  ...
  async function removeCurrent() {
    ...
    try {
      ...
      // 삭제 응답은 null 이라 남은 이미지를 알려 주지 않고, 뒤 장들의 정렬 번호도 앞으로 당겨진다.
      // 번호를 계산해 맞히지 않고 목록을 다시 받아 서버가 준 번호로 통째로 맞춘다.
      // 재동기화 전에 지워진 자리를 비운다 — 그대로 두면 목록 조회가 실패했을 때 showRetry 가
      // 거짓이 되어, 이미 지운 이미지를 계속 보면서 다시 시도할 방법도 없다.
      cacheRef.current.clear();
      setCurrent(null);
      setTotalCount(null);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/*
          캡션은 있든 없든 항상 한 줄 자리를 차지한다 — 조건부로 그리면 캡션을 붙이는 순간
          카드가 커지고 같은 행의 블록 높이까지 딸려 움직인다. truncate 로 한 줄에 가둔다.
        */}
        <p
