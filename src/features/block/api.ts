// 블록 도메인 API — 블록 자체(생성·수정·이동·삭제)와 유형별 내용(체크리스트·텍스트·이미지·문서)을 함께 다룬다.
...
// 409 를 실패로 끝내면 그 블록은 영영 삭제할 수 없다. 확인 다이얼로그가 필수다.
...
// 낙관적 락 — 409 면 재조회·덮어쓰기(overwrite: true)를 사용자에게 묻는다.
// 출발·도착 양쪽 EDITOR 여야 한다 (STEP_EDIT_DENIED).
// 옮기면 이슈 연결이 끊긴다 — 응답 unlinkedIssueCount 를 화면이 알려야 한다.
...
// 부분 수정이 아니라 전체 내용을 보낸다.
// 낙관적 락 (2026-08-11) — body.version 은 블록 목록에서 받은
// detail.version 이다 (block.version 이 아니다). 늦으면 409 이고,
// 부르는 쪽이 재조회·덮어쓰기(overwrite: true)를 사용자에게 묻는다.
// 응답 version 은 저장 후의 새 값이라 화면에 꽂아야 다음 저장이 통과한다.
...
// 목록 조회 API 가 없어 캐러셀 이동마다 한 번씩 부른다.
...
// 편집 권한이 필요하다. 열람만 가능한 사용자에게는 403 이 온다 —
...
// 이미지 복구 — 다건. (명세 110번)
// 권한을 이미지가 속한 스텝별로 확인하므로 보낸 것이 다 돌아오지 않을 수 있다.
// 호출 측은 응답 images[] 에 담겨 온 것만 목록에서 지워야 한다.
// 복구된 이미지는 원래 자리가 아니라 블록 맨 뒤에 붙는다 (orderIndex 재부여).
...
// 파일 영구 삭제(104번)와 달리 확인 문자가 없다 — 화면 확인 모달이 유일한 방어선이다.
// 응답이 null 이라 몇 건이 지워졌는지 알 수 없다. 호출 후 휴지통을 다시 읽는다.
// 본문 있는 DELETE 라 프록시가 본문을 버리면 실패한다 (.ai/API.md 이미지 공통 절).
