...
// 낙관적 락은 남아 있는 항목의 변경만 잡아, 그 사이 남이 지우거나 새로 올린 것은 버전으로 드러나지 않는다.
// 새로 올라온 것은 우리 배열에 없어 저장하는 순간 삭제되므로, 저장 직전에 목록을 한 번 더 읽어 대조한다.
...
export default function ImageEditModal({
  ...
  async function save() {
    ...
    // 버전 없이 보내면 400 이다. 한 장이라도 없으면 배열 전체가 막히므로,
    // 부분만 보내지 않고 저장 자체를 멈추고 새로고침을 안내한다.
    ...
    // 저장 직전에 목록을 한 번 더 읽어 식구가 그대로인지 본다 (sameMembers 참고).
    // 다르면 저장하지 않고 최신 목록으로 갈아 끼운다 — 그대로 보내면 남이 방금 올린 이미지가 조용히 삭제된다.
    try {
    ...
    try {
      ...
      // 남이 먼저 저장했다 — 덮어쓰기 API 가 없어 재조회가 유일한 출구다.
