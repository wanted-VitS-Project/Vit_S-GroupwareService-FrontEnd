...
// CSR - 텍스트 블록 편집 모달. 생성 직후에도 같은 모달이 빈 내용으로 곧바로 열린다.
// 낙관적 락이라 받은 version 을 실어 보내고, 409 면 조용히 삼키지 않고 재조회/덮어쓰기를 묻는다.
// 취소(Esc·배경)는 재조회 쪽으로 흘려 잘못 눌러도 남의 값이 지워지지 않게 한다.
// 임시저장은 편집 중인 본문을 localStorage 에 남기는 장치다(textDraft.ts) — 저장하지 않고 나가거나
// 충돌로 저장이 막혔을 때 쓴 글을 잃지 않게 한다. 되살릴지 버릴지는 언제나 사용자가 고른다.
// 타이핑 중 자동으로 남기지 않고 나갈 때 한 번만 묻는다. 지금 내용이 이미 임시저장함에 있으면
// 묻지 않고 그냥 닫는다 — 이미 안전한데 또 묻는 것은 확인창을 읽지 않게 만든다.
...
interface TextBlockModalProps {
  ...
  /** 낙관적 락 버전(블록 목록의 detail.version). 없으면 저장을 막고 새로고침을 안내한다 */
  version?: number;
  onClose: () => void;
  /** 저장 성공 시 화면에 반영할 내용·새 버전 */
  onSaved: (content: string, version?: number) => void;
  /** 409 에서 재조회를 골랐을 때 — 블록 목록을 다시 읽게 한다 */
  onRefetch: () => void;
}
...
export default function TextBlockModal({
  ...
  >(null);
  // TipTap 이 최초로 파싱·직렬화한 값 — 원문 표기 차이를 수정으로 오인하지 않는다.
  ...
  // 에디터를 다시 마운트시키는 열쇠. MarkdownEditor 는 useEditor({ content }) 로 처음 값만 읽어
  // value 를 바꿔도 화면이 따라오지 않는다 — 초안을 되살릴 때는 통째로 다시 만든다.
  ...
  // 초안을 뜬 뒤 남이 본문을 고쳤는지 — 되살리면 그 변경을 덮어쓰게 된다.
  ...
  // 나가기. 묻는 건 잃을 게 있을 때만이다 — 고친 게 없거나 지금 내용이 이미
  // 임시저장함에 있으면 확인창 없이 닫는다.
  ...
  async function save(overwrite = false) {
    ...
    try {
      ...
      // 서버에 들어간 내용과 같은 초안만 걷어낸다 — 통째로 비우면 따로 남겨 둔 다른 안까지 사라진다.
  ...
  return (
    <>
      <Modal
        title="텍스트 블록 편집"
        // 저장 중에는 ESC·백드롭 클릭까지 막는다 — 닫은 뒤 응답이 도착하면
        // 카드 내용이 예고 없이 바뀐다.
      ...
      >
        ...
        <MarkdownEditor
          ...
          onReady={(normalized) => {
            ...
            // 비교 기준은 첫 마운트의 서버 본문이다 — 초안을 되살려 다시 만든 에디터의 값으로
            // 갈아치우면 되살린 내용이 "수정 없음" 이 되어 저장이 막힌다.
      ...
      {confirmation === 'conflict' && (
        /* 409 를 조용히 삼키면 저장된 줄 안다. 임시저장이 있어야 "덮어쓰긴 부담스럽고 내 글도 버리기 싫다" 는 자리가 생겨 선택지가 셋이다 */
        <ChoiceDialog
          ...
          choices={[
            ...
            },
            {
              ...
              onSelect: () => {
                ...
                setConfirmation(null);
                // 남기지 못했으면 재조회도 닫기도 하지 않는다 — 닫으면 "임시저장하고" 를
                // 골랐는데 글이 사라지고, 재조회가 서버 본문을 덮어 되돌릴 길도 없다.
                if (!kept) return;
        ...
        <ChoiceDialog
          ...
          choices={[
            ...
            },
            {
              ...
              // "이번 편집 버리기" 로 없애면 안 된다. 비우기는 전체 비우기가 맡는다.
              onSelect: onClose,
            },
            {
              label: '계속 편집',
...
// 선택지가 셋 이상인 확인 창. AlertDialogTwoButton 은 확인·취소 둘뿐이라
// "저장 / 임시저장 / 계속 편집" 같은 자리에는 쓸 수 없다.
// 버튼을 세로로 쌓아 각 선택의 결과를 한 줄씩 적고, Esc·배경 클릭은 가장 안전한 쪽으로 흘린다.
