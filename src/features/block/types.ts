...
// 백엔드 enum 에는 두 값이 남아 있을 수 있다. 그래서 block.type 은
...
export interface BlockTypeOption {
  ...
  // 유형을 구별하는 고유 액센트라 시맨틱 토큰으로 뭉갤 수 없다 —
...
export const BLOCK_TYPES: BlockTypeOption[] = [
  ...
  },
  {
    ...
    border: '#C6D2FF',
    /*
     * 여기만 토큰을 참조한다 — 다른 블록의 색은 서로를 구별하기 위한 식별색이지만,
     *    AI 블록의 아이콘색은 화면 곳곳에서 쓰는 AI 브랜드색이라 한 곳에서 관리한다.
     */
...
export interface StepBlock {
  ...
  colSpan: number;
  // 타입별 상세 — 구조가 타입마다 다르다.
  // FILE 의 { fileCount } 만 확인됐다. 나머지는 확인 필요.
  detail: unknown;
  linkedIssueTotal: number;
  linkedIssueDone: number;
  // 낙관적 락 버전 (2026-08-11 신설).
  // 선택 필드로 둔다 — 조회 응답에 실린다는 계약은 나왔지만 실서버 확인 전이다.
...
export interface MoveBlockResponse {
  ...
  stepId: number;
  // 끊긴 이슈-블록 연결 수.
  // 0 이 아니면 사용자에게 알려야 한다 — 블록과 이슈는 같은 스텝이어야 해서 끊긴다.
...
export interface TextBlockDetail {
  ...
  content: string;
  // 낙관적 락 버전 (2026-08-11 신설).
  // block.version 과 다른 값이다 — text 테이블의 자기 버전이다.
  // 블록 목록 조회의 TEXT 상세에 함께 실려 온다.
  // 선택으로 둔다 — 없으면 화면이 저장을 막고 새로고침을 안내한다.
  version?: number;
}
...
// 낙관적 락 — version 필수(없으면 400 TEXT_VERSION_REQUIRED),
...
export interface BlockImage {
  ...
  // 캡션은 보여 주는 문구, originalName 은 파일명이라 둘 다 뜻을 보장하지 않는다.
...
// 스크린리더에 읽어 줄 문구.
// altText 가 가장 정확하다. 없을 때 캡션·파일명으로 떨어지는 것은 차선이고,
...
// 활성 목록(107번)에 있던 imgBlockId 가 없다 — 어느 블록에서 지워졌는지 모른다.
...
// 블록이 담을 수 있는 총 장수에는 제한이 없다 — 요청 한 번의 상한일 뿐이다.
...
// 올릴 수 있는 형식. 화면 안내 문구(JPG·PNG·GIF·WEBP)와 같은 목록이다.
// image/* 로 두면 SVG·BMP 처럼 안내에 없는 형식이 통과한다
// (특히 SVG 는 스크립트를 품을 수 있어 저장소에 그대로 올리면 안 된다).
// 프론트 검사는 편의일 뿐이다 — 서버가 같은 목록으로 독립 검증해야 한다.
...
// 첫 이미지를 담는 키 이름이 확정되지 않아 세 모양을 모두 읽는다 —
...
export interface UpdateImageItemsRequest {
  ...
  // 항목마다 version 이 필수다 (없으면 400 IMAGE_VERSION_REQUIRED).
...
export interface BlockLayout {
  ...
  colSpan: number;
  // 낙관적 락 버전.
  // 요청에는 필수(BlockLayoutOrder), 응답에는 저장 후의 새 값이 온다.
...
// 스텝의 배치 전체를 보낸다 — 옮긴 블록만 보내면 나머지가 지워진다.
...
export interface UpdateBlockResponse {
  blockId: number;
  title: string | null;
  // deleted 가 없을 수 있다 — 46번 명세에 이 필드가 명시돼 있지 않다.
  // 그래서 응답 경계에서는 선택 필드로 받고, 화면 상태에 넣기 전에
  ...
  updatedAt: string;
  /** 저장 후의 새 값. 화면 상태를 이 값으로 교체하지 않으면 다음 저장이 또 409 다 */
  version: number;
}
