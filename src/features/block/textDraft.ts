// 텍스트 블록 임시저장함 — 편집 중인 본문을 localStorage 에 여러 개 남긴다.
// 저장하지 않고 나가거나 낙관적 락 충돌로 저장이 막혔을 때 쓴 글을 잃지 않게 하는 장치다.
// 초안은 이 브라우저의 것이라 다른 기기·다른 사용자와 공유되지 않는다.
// 초안은 사용자가 남길 때만 생긴다 — 임시저장 버튼·모달 이탈·충돌 확인창, 이 세 자리에서만 쓴다.
// sessionStorage 가 아니라 localStorage 다 — 탭을 닫았다 열어도 남아야 "실수로 창을 닫았다" 를 구제한다.
// 저장소 접근은 전부 실패할 수 있다 (사생활 보호 모드·용량 초과). 부가 기능이라 실패해도 편집은 막지 않는다.

const KEY_PREFIX = 'vit-s:text-draft:';

// 한 블록이 들 수 있는 초안 수. 넘치면 가장 오래된 것을 버린다 —
// 무한히 쌓으면 저장소 용량을 텍스트 하나가 다 먹고 목록도 훑을 수 없게 길어진다.
...
export interface TextDraft {
  ...
  /** 마크다운 원문 */
  content: string;
  /** 초안을 뜬 시점의 낙관적 락 버전. 지금 서버 버전과 다르면 그 사이 남이 본문을 고친 것 */
  version?: number;
  /** ISO 문자열 */
  savedAt: string;
}
...
// 초안이 한 칸이던 시절의 단일 객체, 자동/직접을 나누던 시절의 kind 필드 같은 예전 값도 읽어 준다.
...
// 이 블록의 초안 목록을 최신순으로 읽는다. 없거나 읽을 수 없으면 빈 배열.
...
// 목록을 저장소에 쓴다. 성공하면 저장된 목록, 실패하면 null (화면이 "임시저장 못 함" 을 알린다).
function write(
  txtId: number | string,
  drafts: TextDraft[],
): TextDraft[] | null {
...
// 초안을 목록에 쌓는다. 같은 내용이 이미 있으면 새로 만들지 않고 그것을 최신으로 올린다 —
...
// 한 칸만 지우고 남은 목록을 돌려준다 (쓰기 실패면 null).
...
// 초안을 뜬 시각 표기. 오늘이면 '오후 3:14', 다른 날이면 '8월 12일 오후 3:14' —
// 며칠 전 초안을 오늘 것으로 착각하면 안 된다.
