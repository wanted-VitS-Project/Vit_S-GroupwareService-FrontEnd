...
// 분석 하나를 끝날 때까지 지켜본다.
// 서버가 완료를 밀어주는 통로(WebSocket·SSE)가 없어 폴링한다 — 요청 직후 15초는 쉬고
// (그 구간은 거의 항상 PROCESSING 이라 왕복이 낭비다) 그 뒤부터 3초 간격으로 본다.
// justRequested 는 방금 이 화면에서 요청을 넣었다는 뜻이다 — 이미 돌고 있던 분석을
// 나중에 열었을 때는 15초를 기다릴 이유가 없어 곧바로 조회한다.
export function useAnalysisPolling(
  ...
  // 폴링 루프 안에서 읽는 값이라 ref 로 둔다 — state 면 매 조회마다 effect 가 다시 돌아
  // 타이머가 초기화된다.
  const shouldStop = useRef(false);

  // 보던 분석이 바뀌면 렌더 중에 값을 비운다. effect 에서 비우면 한 프레임 동안
  // 이전 분석의 결과가 새 분석의 것처럼 남아 재실행 직후 옛 결과가 깜빡인다.
  ...
  useEffect(() => {
    ...
    // 조회가 도는 중인지. 응답을 기다리는 동안에는 대기 타이머가 없어, 그때 탭을 껐다 켜면
    // resume 의 clearTimeout 이 아무것도 못 지우고 두 번째 루프가 함께 돈다.
    let isPolling = false;
    // 연속 실패 횟수 — 서버가 죽어 있을 때 3초마다 두드리지 않도록.
    let failureCount = 0;

    // 지연 판정에 쓸 경과 시간. 화면을 연 시각이 아니라 분석을 요청한 시각이 기준이어야 한다 —
    // 이미 돌던 분석을 나중에 열면 화면 기준으로는 방금 시작한 것처럼 보인다.
