// 비타메이트 AI 블록 도메인 타입.
// 채팅형이 아니다 — 검토 유형·세부 카테고리를 고르고 문서를 기준(REFERENCE)/대상(TARGET)으로
// 나눠 선택한 뒤, 서버가 준 기본 프롬프트를 확인·보완해 분석을 요청한다.
...
// 문서의 역할 — REFERENCE 는 비교 기준(매뉴얼·산정기준·RFP), TARGET 은 검토받을 문서(제안서·보고서).
...
// 이력 목록의 한 줄. documents·result·citations 가 없어 상세는 단건 조회로 받는다.
...
// AI 블록의 detail. "블록의 최신 분석" 전용 조회 API 가 없어,
// latestAnalysisId 가 실려 오면 곧바로 단건 조회로 가고 없으면 이력 목록의 첫 건으로 대체한다.
...
// 문서 인덱싱(indexStatus) 상수·헬퍼는 features/file/types.ts 에 있다 — 문서 업로드 블록도 같은 값을 쓴다.
...
// API 가 최종 문자열 하나만 받으므로 카테고리별로 나누지 않는다.
...
// high 는 값이 어긋남, medium 은 있어야 할 것이 비어 있음, low 는 그 외(표기·권장).
...
// 요약·경고 구획을 이루는 한 덩이.
// 문자열 하나로 이어 붙이면 소제목과 문단·목록이 한 줄로 뭉개져 원문의 구조가 사라진다.
export interface ResultBlock {
  kind: 'heading' | 'paragraph' | 'item';
  text: string;
  /** heading 의 크기 1~6 (# 개수). 기호 없는 꼴은 3 으로 본다 */
  level?: number;
}
...
// 홀로 선 제목 줄이면 그 제목과 크기를 준다.
// # 개수를 흘려버리면 h1 과 h4 가 같은 모양으로 그려져 원문이 세워 둔 층이 사라진다.
// 기호 없는 꼴은 크기를 알 수 없어 중간(3)으로 둔다.
...
// 지적 사항 한 줄을 제목/상세로 가른다. 굵은 글씨·줄표·콜론 세 꼴을 모두 받는다.
...
// result 는 자유 문자열이라 서버가 형식을 보장하지 않는다 — 나눠지지 않으면 null 을 주고
// 호출부는 마크다운 원문을 그대로 그린다.
export function parseResult(result: string): ParsedResult | null {
  ...
  // 블록을 쌓는 구획은 요약·경고 둘뿐이다 (지적 사항은 findings 가 따로 받는다).
  // 타입에서 findings 를 빼 둔 것은 실수를 막기 위해서다 — 예전에는 Bucket 을
  ...
  for (const line of lines) {
     ...
     *
     * 예전에는 첫 줄이 목록이 아니면 아래로 흘러가 요약에 쌓였다 — ## 지적 사항
