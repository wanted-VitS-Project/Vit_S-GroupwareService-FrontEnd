...
/**
 * 페이지 접근 등급.
 * NONE 은 메뉴는 보이되 들어갈 수 없다는 뜻이다.
 */
export type PagePermission = 'NONE' | 'VIEWER' | 'EDITOR';

/** 부여 화면에서 고를 수 있는 등급. NONE 은 회수로 표현한다 */
export type GrantablePermission = Exclude<PagePermission, 'NONE'>;

/**
 * 접근 근거.
 * GRANTED 만 회수할 수 있다. 나머지는 전역 권한 · 기본값이다.
 */
/**
 * 권한이 어디서 왔는지.
 * NONE 만 서버 값이 아니라 권한 없는 사원을 같은 표에 세우려고 화면이 만든다.
 */
