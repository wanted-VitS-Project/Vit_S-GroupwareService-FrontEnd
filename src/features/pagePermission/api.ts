...
/**
 * 페이지 접근 가능자 목록 (ADMIN).
 * 명시 부여자 + 전역 권한 열람자를 **함께** 준다 — `revocable` 로 갈린다.
 */
 ...
 *
 * ⚠️ **전체 교체가 아니다** — 요청에 없는 사용자는 그대로 남는다.
 * 회수는 `revokePagePermission` 으로 한 명씩 한다.
 */
...
/** 권한 회수. 전역 권한이 있으면 회수해도 계속 보인다 (`stillAccessible`) */
