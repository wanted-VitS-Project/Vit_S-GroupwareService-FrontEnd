...
/**
 * 결재 관리 목록. scope 에 따라 대상이 달라진다.
 * (drafted 내가 기안 · pending 내 차례 · all 전체)
 */
...
/** 결재 상세. 항상 현재 회차를 준다. 이전 회차는 getRevision() 을 쓴다 */
...
/**
 * 회차 상세. 기안자 · 해당 회차 결재자 · MASTER 만 볼 수 있다.
 * 차례가 오지 않은 결재자는 403 으로 막힌다.
 */
...
/** 재상신 회차 생성. 이미 DRAFT 회차가 있으면 그대로 돌려준다 */
...
/**
 * 상신. 최초 · 재상신 겸용이다.
 * 서버가 전부 재검증하므로 프론트 검증은 왕복을 줄이는 용도다.
 */
 ...
 * 업로드 자체는 파일 도메인 소관이라 여기서는 fileVersionId 만 넘긴다.
 */
export function addDocument(
  ...
  // 서버가 파일명을 빼고 줄 수 있어 필수로 좁히지 않는다
...
/** 결재 문서 제거. DRAFT 회차에서만 되고 이력에 남지 않는다 */
...
/**
 * 결재 승인. 대상은 결재가 아니라 결재선이며 ACTIVE 일 때만 가능하다.
 * 응답의 approvalCompleted 가 true 면 결재 전체가 끝난 것이다.
 */
...
/** 결재 반려. 이후 대기 단계는 모두 취소되고 결재가 반려로 종료된다 */
...
/** 결재선 등록 · 수정. 전체 치환이라 한 명만 바꿔도 전체 목록을 보낸다 */
