/**
 * 결재 도메인 타입 (.ai/API.md 결재 절).
 * 결재 > 회차 > 결재선 · 문서 구조이며 회차는 상신할 때마다 새로 쌓인다.
 */

import type { ApprovalStatusCode } from '@/constants/status';

/** 결재 전체 · 회차 공용 상태. 값과 라벨은 constants/status.ts 에 있다 */
export type ApprovalStatus = ApprovalStatusCode;

/** 결재선 한 명의 처리 상태 */
export type ApprovalLineStatus =
  'WAITING' | 'ACTIVE' | 'APPROVED' | 'REJECTED' | 'CANCELED';

export interface ApprovalLine {
  lineId: number;
  /** 사번 */
  approverId: string;
  approverName: string;
  approverPosition: string | null;
  approverDepartment: string | null;
  /** 결재 순번 */
  order: number;
  /** 아래 3개는 응답에 따라 빠질 수 있어 선택 필드로 둔다 */
  status?: ApprovalLineStatus;
  /** 승인 · 반려 의견 */
  opinion?: string | null;
  processedAt?: string | null;
}

/** 결재 상세의 결재선. 처리 상태가 항상 온다 */
export interface ApprovalDetailLine extends ApprovalLine {
  status: ApprovalLineStatus;
  opinion: string | null;
  processedAt: string | null;
  /** 이 결재자가 더는 결재할 수 없는 상태인지 (퇴사 · 이탈 등) */
  approverUnavailable?: boolean;
}

/** 결재가 만들어진 원본 위치. '원본 블록 보기' 이동에 쓴다 */
export interface ApprovalBlockOrigin {
  blockId: number;
  stepId: number;
  projectId: number;
}

/**
 * 결재 관리 상세. 항상 현재 회차를 담는다.
 * 회차 상세와 달리 시각 필드가 없고 대신 blockOrigin 이 있다.
 */
export interface ApprovalDetail {
  revisionId: number;
  revisionNo: number;
  title: string | null;
  content: string | null;
  drafterId: string;
  drafterName: string;
  drafterDepartment: string | null;
  drafterPosition: string | null;
  status: ApprovalStatus;
  documents: ApprovalDocument[];
  lines: ApprovalDetailLine[];
  blockOrigin: ApprovalBlockOrigin;
}

export interface ApprovalDocument {
  documentId: number;
  /** 결재 대상은 파일이 아니라 파일 버전이다 */
  fileVersionId: number;
  /** 문서 추가 응답에만 있다. 없으면 파일 API 로 따로 조회해야 한다 */
  fileName?: string;
  fileSize?: number;
  uploadedAt?: string;
}

/** 회차 상세 응답 */
export interface ApprovalRevision {
  revisionId: number;
  /** 회차 번호. 재상신할 때마다 1씩 오른다 */
  revisionNo: number;
  title: string | null;
  content: string | null;
  drafterId: string;
  drafterName: string;
  drafterDepartment: string | null;
  drafterPosition: string | null;
  status: ApprovalStatus;
  /** DRAFT 회차는 아직 상신 전이라 null */
  submittedAt: string | null;
  /** 진행 중이면 null */
  finishedAt: string | null;
  documents: ApprovalDocument[];
  lines: ApprovalDetailLine[];
  /** 기안자가 더는 상신할 수 없는 상태인지. 참이면 대행 재상신이 필요하다 */
  drafterUnavailable?: boolean;
  /** 대행 기안자. 먼저 재상신한 스텝 EDITOR 가 되며 없으면 null 이다 */
  actingDrafterId?: string | null;
  actingDrafterName?: string | null;
}

/**
 * 결재 목록의 조회 범위. 탭 하나가 값 하나다.
 * all 은 MASTER · ADMIN 전용이라 다른 권한에는 탭을 감춘다.
 */
export type ApprovalScope = 'drafted' | 'pending' | 'all';

/** 결재 목록 쿼리. 값이 있는 것만 실어 보낸다 */
export interface ApprovalListQuery {
  /** 생략하면 서버 기본값 drafted */
  scope?: ApprovalScope;
  status?: ApprovalStatus;
  /** 아래 둘은 scope=all 에서만 적용된다 */
  drafterId?: string;
  approverId?: string;
  /** yyyy-MM-dd */
  fromDate?: string;
  toDate?: string;
  /** 결재 제목 또는 프로젝트명 */
  keyword?: string;
  /** 현재 회차 번호 */
  revisionNo?: number;
  /** 0부터 시작한다 (사원 목록과 다르다) */
  page?: number;
  /** 기본 10 */
  size?: number;
}

/** 결재 목록 응답 봉투. page · size 는 오지 않는다 */
export interface ApprovalPage<T> {
  content: T[];
  totalElements: number;
  totalPages: number;
}

/**
 * 목록 행의 결재선 요약. 아바타 줄과 진행 카운트에 쓴다.
 * 회차 상세와 달리 status 가 오고 부서 · 직급 · lineId 는 없다.
 */
export interface ApprovalListLine {
  approverId: string;
  approverName: string;
  order: number;
  status: ApprovalLineStatus;
}

/** 결재 목록 한 줄 */
export interface ApprovalListItem {
  approvalId: number;
  title: string | null;
  status: ApprovalStatus;
  /** 현재 회차 번호. 재상신했으면 2 이상이다 */
  currentRevisionNo: number;
  drafterId: string;
  drafterName: string;
  /** 지금 차례인 결재자. 완료 · 반려된 결재는 null */
  currentApproverId: string | null;
  currentApproverName: string | null;
  /** 목록에 프로젝트 > 스텝 경로를 그리는 데 쓴다 */
  projectId: number;
  projectName: string;
  stepId: number;
  stepName: string;
  lines: ApprovalListLine[];
  /** 블록을 만든 시각. 상신 전에도 있다 */
  createdAt: string;
  /** DRAFT 면 null */
  submittedAt: string | null;
  completedAt: string | null;
}

/**
 * 승인 · 반려 요청 본문. 의견은 선택이다.
 * 대상이 결재가 아니라 결재선이며 lineId 는 회차 상세에서 얻는다.
 */
export interface ProcessLineRequest {
  opinion?: string;
}

export interface RejectLineResponse {
  lineId: number;
  status: ApprovalLineStatus;
  processedAt: string;
}

export interface ApproveLineResponse extends RejectLineResponse {
  /** 다음 결재자가 없으면 null 이며 그때 approvalCompleted 가 true 다 */
  nextActiveLineId: number | null;
  /** 마지막 순번의 승인이면 결재 전체가 끝난다 */
  approvalCompleted: boolean;
}

/** 회차 수정 요청. 보낸 필드만 바뀐다 */
export interface UpdateRevisionRequest {
  title?: string;
  content?: string;
}

export interface UpdateRevisionResponse {
  revisionId: number;
  title: string;
  content: string;
  updatedAt: string;
}

/**
 * 재상신 회차 생성 응답.
 * 이전 회차의 제목 · 내용 · 문서를 복사하고 결재선은 반려자부터 재구성한다.
 */
export interface CreateRevisionResponse {
  revisionId: number;
  revisionNo: number;
  status: ApprovalStatus;
  /** 어느 회차를 복사했는지 */
  copiedFromRevisionNo: number;
  title: string | null;
  content: string | null;
  documents: ApprovalDocument[];
  lines: ApprovalLine[];
}

/**
 * 결재 이력 한 줄. 회차를 고르는 데 필요한 값만 온다.
 * 고른 회차의 내용은 회차 상세로 따로 받는다.
 */
export interface ApprovalRevisionSummary {
  revisionId: number;
  /** 1부터 시작하며 재상신마다 1씩 오른다 */
  revisionNo: number;
  status: ApprovalStatus;
  /** DRAFT 회차는 아직 상신 전이라 null */
  submittedAt: string | null;
  /** 진행 중이면 null */
  finishedAt: string | null;
  /** 지금 살아 있는 회차. 목록에 하나만 true 다 */
  isCurrent: boolean;
}

/** 결재 이력 응답. 회차 번호 오름차순이고 페이징이 없다 */
export interface ApprovalRevisionHistory {
  content: ApprovalRevisionSummary[];
}

/** 상신 응답 */
export interface SubmitRevisionResponse {
  approvalId: number;
  revisionId: number;
  revisionNo: number;
  status: ApprovalStatus;
  submittedAt: string;
  /** 상신 직후 활성화된 첫 결재선 */
  firstActiveLineId: number;
}

/** 결재 문서 연결 요청 */
export interface AddDocumentRequest {
  /** 업로드가 끝난 파일 버전만 연결할 수 있다 */
  fileVersionId: number;
}

/** 결재선 등록 요청. 전체 치환이다 */
export interface SetLinesRequest {
  lines: { approverId: string; order: number }[];
}

export interface SetLinesResponse {
  lines: ApprovalLine[];
}

/**
 * 블록 목록 응답의 detail 에서 결재 연결 정보를 꺼낸다.
 * 블록 ID 와 결재 ID 는 다른 값이라 서로 대체하지 않는다.
 */
export interface ApprovalBlockDetail {
  approvalId: number;
  /** 현재 회차. 초안이면 DRAFT, 상신 후면 진행 중인 회차 */
  revisionId: number;
  /** 회차를 받기 전에도 요약을 그릴 수 있게 목록 응답이 함께 내려준다 */
  title?: string | null;
  content?: string | null;
  /** 결재자 교체 · 제외가 필요한 상태인지. 대상 결재선은 회차 상세에서 고른다 */
  requiresApproverReplacement?: boolean;
}

/** 문자열 또는 null 만 통과시킨다 */
function readText(value: unknown) {
  if (typeof value === 'string') return value;
  return value === null ? null : undefined;
}

export function readApprovalBlockDetail(
  detail: unknown,
): ApprovalBlockDetail | null {
  if (typeof detail !== 'object' || detail === null) return null;

  const {
    approvalId,
    revisionId,
    title,
    content,
    requiresApproverReplacement,
  } = detail as {
    approvalId?: unknown;
    revisionId?: unknown;
    title?: unknown;
    content?: unknown;
    requiresApproverReplacement?: unknown;
  };

  // 둘 중 하나라도 없으면 어느 결재의 어느 회차인지 특정할 수 없다
  if (typeof approvalId !== 'number' || typeof revisionId !== 'number') {
    return null;
  }

  return {
    approvalId,
    revisionId,
    title: readText(title),
    content: readText(content),
    // 참이라고 확실할 때만 배너를 띄운다
    requiresApproverReplacement: requiresApproverReplacement === true,
  };
}
